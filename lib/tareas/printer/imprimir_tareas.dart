...
class ImprimirTareas {
  ImprimirTareas();

  static Future<File> generate(List<TareaPendiente> list) async {
    final firmaControlOperacion = MemoryImage(
      (await rootBundle.load(logoApp)).buffer.asUint8List(),
    );

    final imageFirma = MemoryImage(
      (await rootBundle.load(firmaLu)).buffer.asUint8List(),
    );
    ...
    return PdfApi.saveDocument(
      name:
          'listado_tarea_pendientes_${DateTime.now().millisecondsSinceEpoch}_.pdf',
      pdf: pdf,
    );
  }
        ...
        Row(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Text(
                  currentEmpresa.nombreEmpresa ?? 'N/A',
                  style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                ...
                SizedBox(height: 2),
                Text(
                  '${currentEmpresa.adressEmpressa}',
                  style: const TextStyle(fontSize: 8, color: PdfColors.grey700),
                ),
                Row(
                  children: [
                    Text(
                      'Tel: ${currentEmpresa.telefonoEmpresa}',
                      style: const TextStyle(
                        fontSize: 8,
                    ...
                    Text(
                      'Cel: ${currentEmpresa.celularEmpresa}',
                      style: const TextStyle(
                        fontSize: 8,
                        color: PdfColors.grey700,
                      ),
  ...
  static Widget tableProducts(List<TareaPendiente> listProduct) {
    final headers = [
      'USUARIO',
      'FECHA',
      'TITULO',
  ...
  static Widget buildFooter(image) => Column(
    mainAxisAlignment: MainAxisAlignment.start,
    crossAxisAlignment: CrossAxisAlignment.center,
    children: [Image(image, height: 100, width: 100)],
  );
